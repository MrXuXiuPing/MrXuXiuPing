...
////    }
    public static void testProductIdByMoreThread(int dataCenterId, int workerId, int n) throws InterruptedException {
        ...
        Map<String, SnowFlake> idFactories = new HashMap<>();
        for (int i = 0; i < 10; i++) {
            //用线程名称做map key.
            idFactories.put("snowflake" + i, new SnowFlake(workerId, threadNo++));
        }
        for (int i = 0; i < 10; i++) {
            Thread temp = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    SnowFlake idWorker = idFactories.get(Thread.currentThread().getName());
                    for (int j = 0; j < n; j++) {
    ...
    public static void testProductId(int dataCenterId, int workerId, int n) {
        ...
        SnowFlake idWorker2 = new SnowFlake(workerId + 1, dataCenterId);
